...
#ifdef SERNUM_DES_LOG
#define LOGE(fmt, args...) __android_log_print(ANDROID_LOG_ERROR, SERNUM_DES_LOG, fmt, ##args)
...
int Serinum_En_DES(char* s1,char* des){
	unsigned char src16[16+1],key16[16+1],key48[48+1],dest16[16+1];
	memset(key16,0,sizeof(key16));
	snprintf((char*)key16,17,"TLCYTLKGTLCYTLKG");

	memset(src16,0,sizeof(src16));
	LOGE("Serinum_En_DES sizeof(src16):%d\n",sizeof(src16));
	memcpy(src16,s1,14);
	memset(dest16,0,sizeof(dest16));
	Do_DES(src16,key16,dest16,'e');
	memcpy(des,dest16,17);
	return 0;
}
int Serinum_De_DES(char* s1,char* des){
	unsigned char src16[16+1],key16[16+1],key48[48+1],dest16[16+1];
	memset(key16,0,sizeof(key16));
	LOGE("Serinum_De_DES sizeof(key16):%d\n",sizeof(key16));
	snprintf((char*)key16,17,"TLCYTLKGTLCYTLKG");
	memset(src16,0,sizeof(src16));
	memcpy(src16,s1,16);
	memset(dest16,0,sizeof(dest16));
...
//做3DES加密或解密运算
int Do_3DES(unsigned char* strSrc, char* strKey, char* strDest, char flag)
{ 
