 ...
 * Base64加密算法，无换行符
 */
signed char* base64_encode(const char *old_string,char *alphabet, char fillChar,int lenght)
{
 ...
 * Base64加密算法，有换行符
 */
signed char* base64_encode(const char *old_string,char *alphabet, char fillChar,int maxCharPreLine,int lenght)
{
    temp_fill_char = fillChar;
...
int decode_Char(int c,char * alphabet )
{
    int i;
    for(i = 0; i < 64; i++)
    {
 ...
 * Base64解码
 */
signed char* base64_decode(const char *old_string,char *alphabet, char fillChar,int lenght)
{
    temp_fill_char = fillChar;
