...
public class AgcUtils {
    ...
    static {
        System.loadLibrary("agcjni");
    }

    //agc实例
    private long agcInstance = -1;
    //agc配置
     ...
     * @param fs 采样率
     */
    public native int init(long agcInstance,int minLevel,int maxLevel,int agcMode,int fs);

    /**
     * 销毁agc实例
     */
    public native int free(long agcInstance);
     ...
     *                          : -1 - Error
     */
    public native int process(long agcInstance,short[] inNear,int num_bands,int samples,short[] out,int inMicLevel,int outMicLevel,int echo,int saturationWarning);
     ...
     *                            : -1 - Error
     */
    public native int setConfig(long agcInstance,WebRtcAgcConfig agcConfig);


    public native int addFarend(long agcInstance,short[] inFar,int samples);
    public native int addMic(long agcInstance,short[] inMic,int num_bands,int samples);
    ...
    public int agcProcess(short[] inbuf,short[] outbuf,int samples){
        return agcProcess(inbuf,0,samples,outbuf,0,0,0,0);
    }
